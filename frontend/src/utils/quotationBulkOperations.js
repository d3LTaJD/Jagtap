/**
 * QUOTATION BULK OPERATIONS & SPEC CLONING (FRONTEND ENGINE)
 * Shared calculation and safe selective cloning for React components.
 */
...
export function applyBulkPricing(items, { targetIndices = [], action, value = 0, adjustType = 'percent' }) {
...
export function applySelectiveSpecCloning(items, {
  sourceIndex = 0,
  targetScope = 'all',
  targetIndices = [],
  categories = {
  ...
  return items.map((targetItem, idx) => {
    if (idx === sourceIndex) return targetItem;
